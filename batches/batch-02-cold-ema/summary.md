...
399 TAO → the same open succeeds. Long-side mirror (`derivative_cold_ema.rs::long_open_cold_ema_live_alpha_bypasses_
capacity_cap`) and a rollback/atomicity probe (`derivative_rollback.rs`) are **in progress by a parallel agent** in the worktree.

**Severity:** re-enables a defended attack class, but pre-launch, fresh-subnet-scoped, and not a direct drain (at the
0.5/0.5 baseline `K0=N`, so an oversized cold-window position still returns ~P on self-close). **Open follow-up:** does
the cold window compose into a cross-state drain (open at a pumped reserve / close at the restored reserve)? — unproven.
